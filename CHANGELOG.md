...
- `kserve-llmisvc-resources`: a ServiceMonitor for the llmisvc controller (giantswarm/giantswarm#36711). The chart shipped a metrics Service and no ServiceMonitor at all, so nothing scraped it even where Prometheus Operator CRDs are served; `kserve.llmisvc.controller.serviceMonitor.enabled` is off by default, `.interval` and `.labels` are forwarded to it. `kserve.llmisvc.controller.metricsBindAddress` moves from `127.0.0.1` to empty (`--metrics-addr=:8443`, every interface): the loopback default made the metrics port unreachable from the Service and therefore from any ServiceMonitor, the same class of gap the kagent controller had before kagent-dev/kagent#2886.
